...
func (c *Scheduler) Stop() {
	...
	for workerID, worker := range c.workers {
		close(worker.readyJobs)
		close(worker.almostReadyJobs)
		delete(c.workers, workerID)
	}

	close(c.jobs)

	for jobID, pending := range c.fullJobs {
		if pending != nil && pending.Finished != nil {
			close(pending.Finished)
		}
		delete(c.fullJobs, jobID)
	}

	for _, waitObj := range c.scheduled {
		if !waitObj.closed {
			waitObj.closed = true
			close(waitObj.done)
		}
	...
	c.logger.Info("Scheduler stopped gracefully")
}
